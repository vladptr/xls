...
async def cleanup_user_data(user_id: int, guild: discord.Guild):
    ...
    try:
        ...
        db = get_supabase()
        if not db:
            print(f"⚠️ Не удалось подключиться к базе данных для удаления данных пользователя {user_id}")
            return False
        ...
        try:
            db.table("user_registrations").delete().eq("discord_id", str(user_id)).execute()
            deleted_count += 1
        except Exception as e:
        ...
        try:
            db.table("voice_time").delete().eq("user_id", user_id).execute()
            deleted_count += 1
        except Exception as e:
        ...
        try:
            db.table("user_levels").delete().eq("user_id", user_id).execute()
            deleted_count += 1
        except Exception as e:
        ...
        try:
            db.table("voice_sessions").delete().eq("user_id", user_id).execute()
            deleted_count += 1
        except Exception as e:
...
async def check_and_cleanup_left_users():
    ...
    try:
        ...
        db = get_supabase()
        if not db:
            print("⚠️ Не удалось подключиться к базе данных для проверки пользователей")
            return
        ...
        try:
            registrations = db.table("user_registrations").select("discord_id").execute()
        ...
        # Из voice_time
        try:
            voice_time_users = db.table("voice_time").select("user_id").execute()
        ...
        # Из user_levels
        try:
            level_users = db.table("user_levels").select("user_id").execute()
            if level_users.data:
