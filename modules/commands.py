...
async def form(ctx, member: discord.Member = None):
    ...
    try:
        ...
        try:
            ...
            print(f"✅ Форма регистрации отправлена пользователю {member.display_name} ({member.id}) через команду !form")
        except discord.Forbidden:
            ...
            print(f"⚠️ Форма регистрации отправлена в канал для {member.display_name} (DM закрыты)")
        except Exception as e:
        ...
        print(f"❌ Ошибка в команде form: {e}")
